...
async function hazardsUpstream(lat, lon) {
  const q = `[out:json][timeout:12];(node(around:65000,${lat},${lon})[amenity=hospital];node(around:65000,${lat},${lon})[amenity=fuel];node(around:65000,${lat},${lon})[highway=construction];node(around:65000,${lat},${lon})[hazard];way(around:65000,${lat},${lon})[highway=construction];);out center tags;`;
  const endpoints = [UPSTREAM.overpass, UPSTREAM.overpassFallback];
  for (const url of endpoints) {
    try {
      const d = await fetchJSON(url, { method: "POST", headers: { "Content-Type": "text/plain" }, body: q }, 20000);
...
app.get("/api/context", async (req, res) => {
  ...
  if (cached) return res.json(cached);
  try {
    const [weather, hazards] = await Promise.all([weatherUpstream(lat, lon), hazardsUpstream(lat, lon)]);
...
app.post("/api/score", express.json({limit: "10mb"}), async (req, res) => {
  const { routes, weather, hazards, origin, vehicle: rawVehicle } = req.body || {};
  ...
  try {
    ...
    const tiered = await tierRoutes(routes, { weather, hazards, originTerrain, mlClient, vehicle });

    const sessionId = randomId();
    scoreSessions.set(sessionId, { routes, weather, hazards, originTerrain, vehicle }, SCORE_SESSION_TTL_MS);
...
app.post("/api/missions", express.json({ limit: "100kb" }), async (req, res) => {
  ...
  try {
    ...
    res.status(503).json({ error: "Mission storage is temporarily unavailable." });
  }
});
...
app.patch("/api/delivery-stops/:id", express.json({ limit: "20kb" }), async (req, res) => {
  ...
  try {
    ...
    res.status(503).json({ error: "Delivery storage is temporarily unavailable." });
  }
});
