...
let map, mapTiles, satTiles, routeLayer, markerLayer, incidentLayer, hazardLayer, serviceLayer, liveIncidentLayer, trackLayer, reportLayer;
let navTimer=null, navIndex=0, navPath=[], traveledLine=null, navMarker=null, followNav=true;
...
async function upsertTrackedDevice(deviceId, lat, lon, ts, isInitialSnapshot){
  ...
  if(!dev){
    dev = {
      marker: L.circleMarker([lat,lon],{radius:8,color:"#fff",weight:2,fillColor:"#9334e6",fillOpacity:1}).addTo(trackLayer),
...
function bind(){
  ...
  $("demoBtn").onclick=()=>{S.demo=!S.demo;$("demoBtn").textContent=S.demo?T("btn_live"):T("btn_demo");toast(S.demo?T("demo_on"):T("demo_off"));};
...
function renderStops(){
  $("stops").innerHTML=S.stops.map((v,i)=>{
    ...
    const remove=!first&&!last?`<button class="remove-stop" onclick="removeStop(${i})">×</button>`:"";
  ...
  document.querySelectorAll(".search-row input").forEach(inp=>inp.onchange=e=>S.stops[+e.target.dataset.i]=e.target.value.trim());
}
...
async function scoreRoutes(routes, weather, hazards, origin, vehicle){
  const d=await fetchJSON(API.score,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({routes,weather,hazards,origin,vehicle})},20000);
  if(d.error) throw new Error(d.error);
...
async function runAnalysis(){
  ...
  try{
    ...
    const scored=await scoreRoutes(routes,w,hazardsForScore,origin,S.vehicle);
    ...
    S.incidents=places.filter(p=>p.tags.hazard||p.tags.highway==="construction");
    S.hazards=places.filter(p=>p.tags.hazard);
...
function drawMap(a,b){
  ...
  S.incidents.forEach(p=>L.circleMarker([p.lat,p.lon],{radius:7,color:"#c5221f",fillColor:"#c5221f",fillOpacity:.9}).addTo(incidentLayer).bindPopup(`<div class="popup-title popup-risk">${esc(p.tags.name||"Mapped road signal")}</div>${esc(p.tags.description||p.tags.highway||p.tags.hazard||"OSM feature")}`));
  S.hazards.forEach(p=>L.circleMarker([p.lat,p.lon],{radius:7,color:"#b06000",fillColor:"#f9ab00",fillOpacity:.9}).addTo(hazardLayer).bindPopup(`<div class="popup-title">Hazard signal</div>${esc(p.tags.hazard||"Mapped hazard")}`));
...
window.startNav=()=>{
  ...
  const realDev=findRealDeviceNearRoute(r);
  ...
  navMarker=L.marker(navPath[0]).addTo(map);traveledLine=L.polyline([navPath[0]],{color:"#5f6368",weight:6,opacity:.55}).addTo(map);map.setView(navPath[0],15);updateNavInstruction();
  navTimer=setInterval(advanceNav,350);
};
...
function locateMe(){
  ...
  navigator.geolocation.getCurrentPosition(p=>{map.setView([p.coords.latitude,p.coords.longitude],14);L.marker([p.coords.latitude,p.coords.longitude]).addTo(markerLayer).bindPopup("Your current browser location").openPopup()},()=>toast("Location permission was not granted."));
}
