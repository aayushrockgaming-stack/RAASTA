...
const SHELL_CACHE = "raasta-shell-v6";
const DATA_CACHE = "raasta-data-v6";
...
self.addEventListener("fetch", (event) => {
  ...
  if (isShellRequest(url) || (event.request.mode === "navigate" && url.origin === self.location.origin)) {
    event.respondWith(shellStrategy(event.request.mode === "navigate" ? new Request("/index.html") : event.request));
    return;
  }
